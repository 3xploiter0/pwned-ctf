...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            iM6613getXimpl = IntOffset.m6614getYimpl(visibleItem.getOffset());
        } else {
            iM6613getXimpl = IntOffset.m6613getXimpl(visibleItem.getOffset());
        }
        return iM6613getXimpl;
    }

    private final int calculateLineAverageMainAxisSize(LazyGridLayoutInfo layoutInfo) {
        int iM6655getWidthimpl;
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        iM6655getWidthimpl = IntSize.m6654getHeightimpl(visibleItems.get(lineEndIndex).getSize());
                    } else {
                        iM6655getWidthimpl = IntSize.m6655getWidthimpl(visibleItems.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, iM6655getWidthimpl);
                    lineEndIndex++;
                }
