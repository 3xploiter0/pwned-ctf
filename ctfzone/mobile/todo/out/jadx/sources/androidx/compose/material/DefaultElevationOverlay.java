...
final class DefaultElevationOverlay implements ElevationOverlay {
    ...
    public long mo1536apply7g2Lkgo(long color, float elevation, Composer $composer, int $changed) {
        ...
        int $this$dp$iv = Dp.m6484compareTo0680j_4(elevation, Dp.m6485constructorimpl(0));
        if ($this$dp$iv <= 0 || colors.isLight()) {
            foregroundColor = color;
        } else {
            long foregroundColor2 = ElevationOverlayKt.m1561calculateForegroundColorCLU3JFs(color, elevation, $composer, ($changed & 14) | ($changed & 112));
            foregroundColor = ColorKt.m4048compositeOverOWjLjI(foregroundColor2, color);
        }
        if (ComposerKt.isTraceInProgress()) {
