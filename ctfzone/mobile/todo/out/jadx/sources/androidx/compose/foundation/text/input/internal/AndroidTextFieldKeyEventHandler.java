...
public final class AndroidTextFieldKeyEventHandler extends TextFieldKeyEventHandler {
    ...
    public boolean mo1135onPreKeyEventMyFupTE(KeyEvent event, TransformedTextFieldState textFieldState, TextFieldSelectionState textFieldSelectionState, FocusManager focusManager, SoftwareKeyboardController keyboardController) {
        if (super.mo1135onPreKeyEventMyFupTE(event, textFieldState, textFieldSelectionState, focusManager, keyboardController)) {
            return true;
        }
        InputDevice device = event.getDevice();
        if (device != null && device.supportsSource(InputDeviceCompat.SOURCE_DPAD) && !device.isVirtual() && KeyEventType.m5061equalsimpl0(KeyEvent_androidKt.m5069getTypeZmokQxo(event), KeyEventType.INSTANCE.m5065getKeyDownCS__XNY()) && event.getSource() != 257) {
            if (TextFieldKeyEventHandler_androidKt.m1207isKeyCodeYhN2O0w(event, 19)) {
                return focusManager.mo3689moveFocus3ESFkO8(FocusDirection.INSTANCE.m3687getUpdhqQ8s());
            }
            if (TextFieldKeyEventHandler_androidKt.m1207isKeyCodeYhN2O0w(event, 20)) {
                return focusManager.mo3689moveFocus3ESFkO8(FocusDirection.INSTANCE.m3680getDowndhqQ8s());
            }
            if (TextFieldKeyEventHandler_androidKt.m1207isKeyCodeYhN2O0w(event, 21)) {
                return focusManager.mo3689moveFocus3ESFkO8(FocusDirection.INSTANCE.m3683getLeftdhqQ8s());
            }
            if (TextFieldKeyEventHandler_androidKt.m1207isKeyCodeYhN2O0w(event, 22)) {
                return focusManager.mo3689moveFocus3ESFkO8(FocusDirection.INSTANCE.m3686getRightdhqQ8s());
            }
            if (!TextFieldKeyEventHandler_androidKt.m1207isKeyCodeYhN2O0w(event, 23)) {
                return false;
            }
    ...
    public boolean mo1134onKeyEvent6ptp14s(KeyEvent event, TransformedTextFieldState textFieldState, TextLayoutState textLayoutState, TextFieldSelectionState textFieldSelectionState, boolean editable, boolean singleLine, Function0<Unit> onSubmit) {
        if (KeyEventType.m5061equalsimpl0(KeyEvent_androidKt.m5069getTypeZmokQxo(event), KeyEventType.INSTANCE.m5065getKeyDownCS__XNY()) && event.isFromSource(InputDeviceCompat.SOURCE_KEYBOARD) && !TextFieldKeyEventHandler_androidKt.m1206isFromSoftKeyboardZmokQxo(event)) {
            textFieldSelectionState.setInTouchMode(false);
        }
        return super.mo1134onKeyEvent6ptp14s(event, textFieldState, textLayoutState, textFieldSelectionState, editable, singleLine, onSubmit);
    }
}
