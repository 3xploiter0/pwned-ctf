...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo1754rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer $composer, int $changed) {
        long jMo1590defaultColorWaAFU9c;
        ...
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventStart(988743187, $changed, -1, "androidx.compose.material.ripple.Ripple.rememberUpdatedInstance (Ripple.kt:115)");
        }
        ...
        long $this$isSpecified$iv = this.color.getValue().m4013unboximpl();
        if ($this$isSpecified$iv != Color.INSTANCE.m4039getUnspecified0d7_KjU()) {
            jMo1590defaultColorWaAFU9c = this.color.getValue().m4013unboximpl();
        } else {
            jMo1590defaultColorWaAFU9c = theme.mo1590defaultColorWaAFU9c($composer, 0);
        }
        $composer.endReplaceableGroup();
        State color = SnapshotStateKt.rememberUpdatedState(Color.m3993boximpl(jMo1590defaultColorWaAFU9c), $composer, 0);
        State rippleAlpha = SnapshotStateKt.rememberUpdatedState(theme.rippleAlpha($composer, 0), $composer, 0);
        RippleIndicationInstance instance = mo1754rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, color, rippleAlpha, $composer, ($changed & 14) | (($changed << 12) & 458752));
    ...
    public boolean equals(Object other) {
        ...
        return (other instanceof Ripple) && this.bounded == ((Ripple) other).bounded && Dp.m6490equalsimpl0(this.radius, ((Ripple) other).radius) && Intrinsics.areEqual(this.color, ((Ripple) other).color);
    }

    public int hashCode() {
        int result = Boolean.hashCode(this.bounded);
        return (((result * 31) + Dp.m6491hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
