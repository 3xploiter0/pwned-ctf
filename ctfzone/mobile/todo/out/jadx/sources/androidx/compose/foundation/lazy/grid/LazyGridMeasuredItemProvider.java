...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public final LazyGridMeasuredItem m825getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        if (Constraints.m6436getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m6440getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m6435getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            crossAxisSize = Constraints.m6439getMinHeightimpl(constraints);
        }
        return mo820createItemO3s9Psw(index, key, contentType, crossAxisSize, mainAxisSpacing, placeables, constraints, lane, span);
    }
