...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo91measure3p2s80s(MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        if (it != null && it.getValue().intValue() != Integer.MAX_VALUE) {
            float $this$fastRoundToInt$iv = it.getValue().floatValue() * this.fraction;
            iRound = Math.round($this$fastRoundToInt$iv);
        } else {
            iRound = Integer.MAX_VALUE;
        }
        int width = iRound;
        State it2 = this.heightState;
        if (it2 != null && it2.getValue().intValue() != Integer.MAX_VALUE) {
            float $this$fastRoundToInt$iv2 = it2.getValue().floatValue() * this.fraction;
            iRound2 = Math.round($this$fastRoundToInt$iv2);
        } else {
            iRound2 = Integer.MAX_VALUE;
        }
        int height = iRound2;
        long childConstraints = ConstraintsKt.Constraints(width != Integer.MAX_VALUE ? width : Constraints.m6440getMinWidthimpl(constraints), width != Integer.MAX_VALUE ? width : Constraints.m6438getMaxWidthimpl(constraints), height != Integer.MAX_VALUE ? height : Constraints.m6439getMinHeightimpl(constraints), height != Integer.MAX_VALUE ? height : Constraints.m6437getMaxHeightimpl(constraints));
