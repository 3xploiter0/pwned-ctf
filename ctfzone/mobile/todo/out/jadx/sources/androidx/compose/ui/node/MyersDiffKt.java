...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m5448constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] backward = CenteredArray.m5448constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] snake2 = Snake.m5634constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            ...
            int[] snake3 = snake2;
            boolean found = m5533midPointq5eDKzI(oldStart, oldEnd2, newStart, newEnd, cb, forward, backward, snake2);
            if (!found) {
                snake2 = snake3;
                oldEnd = max;
                forward = forward2;
            } else {
                if (Snake.m5637getDiagonalSizeimpl(snake3) <= 0) {
                    snake = snake3;
                } else {
                    snake = snake3;
                    Snake.m5632addDiagonalToStackimpl(snake, diagonals);
                }
                stack.pushRange(oldStart, Snake.m5642getStartXimpl(snake), newStart, Snake.m5643getStartYimpl(snake));
                stack.pushRange(Snake.m5638getEndXimpl(snake), oldEnd2, Snake.m5639getEndYimpl(snake), newEnd);
                snake2 = snake;
                oldEnd = max;
                forward = forward2;
            }
    ...
    private static final boolean m5533midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        ...
        CenteredArray.m5454setimpl(forward, 1, oldStart);
        CenteredArray.m5454setimpl(backward, 1, oldEnd);
        int d = 0;
        while (d < max) {
            int d2 = d;
            boolean found = m5532forward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found) {
                return true;
            }
            boolean found2 = m5531backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake);
            if (found2) {
                return true;
            }
            d = d2 + 1;
    ...
    private static final boolean m5532forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int i = oldEnd;
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m5451getimpl(forward, k + 1) > CenteredArray.m5451getimpl(forward, k - 1))) {
                startX = CenteredArray.m5451getimpl(forward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m5451getimpl(forward, k - 1);
                x = startX + 1;
            ...
            CenteredArray.m5454setimpl(forward, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + i2 && backwardsK <= d - 1) {
                    if (CenteredArray.m5451getimpl(backward, backwardsK) > x) {
                        i2 = 1;
                    } else {
    ...
    private static final boolean m5531backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int startX;
        int x;
        int oldSize = oldEnd - oldStart;
        int newSize = newEnd - newStart;
        ...
        int delta = oldSize - newSize;
        for (int k = -d; k <= d; k += 2) {
            if (k == (-d) || (k != d && CenteredArray.m5451getimpl(backward, k + 1) < CenteredArray.m5451getimpl(backward, k - 1))) {
                startX = CenteredArray.m5451getimpl(backward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m5451getimpl(backward, k - 1);
                x = startX - 1;
            ...
            CenteredArray.m5454setimpl(backward, k, x);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d && CenteredArray.m5451getimpl(forward, forwardsK) >= x) {
