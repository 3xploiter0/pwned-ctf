...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5948getStartimpl(j);
        this.selectionEnd = TextRange.m5943getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m5948getStartimpl(j), TextRange.m5943getEndimpl(j));
    }
    ...
    public final TextRange m1142getCompositionMzsxiRA() {
        if (hasComposition()) {
            return TextRange.m5936boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m5946getMinimpl(deleteRange), TextRange.m5945getMaximpl(deleteRange), "", 0, 0, 24, null);
        long newSelection = EditingBufferKt.m1145updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), deleteRange);
        setSelectionStart(TextRange.m5948getStartimpl(newSelection));
        setSelectionEnd(TextRange.m5943getEndimpl(newSelection));
        if (hasComposition()) {
            long compositionRange = TextRangeKt.TextRange(this.compositionStart, this.compositionEnd);
            long newComposition = EditingBufferKt.m1145updateRangeAfterDeletepWDy79M(compositionRange, deleteRange);
            if (TextRange.m5942getCollapsedimpl(newComposition)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m5946getMinimpl(newComposition);
                this.compositionEnd = TextRange.m5945getMaximpl(newComposition);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1144setHighlightK7f2yys(int type, int start, int end) {
        if (start >= end) {
            throw new IllegalArgumentException("Do not set reversed or empty range: " + start + " > " + end);
        }
        int clampedStart = RangesKt.coerceIn(start, 0, getLength());
        int clampedEnd = RangesKt.coerceIn(end, 0, getLength());
        this.highlight = new Pair<>(TextHighlightType.m1114boximpl(type), TextRange.m5936boximpl(TextRangeKt.TextRange(clampedStart, clampedEnd)));
    }
