...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public final float flingDistance(float velocity) {
        ...
        return ((float) (WindowInsetsConnection_androidKt.PlatformFlingScrollFriction * this.magicPhysicalCoefficient * Math.exp((WindowInsetsConnection_androidKt.DecelerationRate / WindowInsetsConnection_androidKt.DecelMinusOne) * l))) * Math.signum(velocity);
    }
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return (AndroidFlingSpline.FlingResult.m556getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m551flingPositionLfoxSSI(splinePos)) * distance) + initialValue;
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        float splinePos = duration > 0 ? playTimeNanos / duration : 1.0f;
        float distance = flingDistance(initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m557getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m551flingPositionLfoxSSI(splinePos)) * distance) / duration) * 1.0E9f;
    }
}
