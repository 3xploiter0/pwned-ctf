...
public final class RippleHostView extends View {
    ...
    public final void m1760addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1761updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m3762getXimpl(interaction.getPressPosition()), Offset.m3763getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1761updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m1768setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m3831getWidthimpl(size)), MathKt.roundToInt(Size.m3828getHeightimpl(size)));
