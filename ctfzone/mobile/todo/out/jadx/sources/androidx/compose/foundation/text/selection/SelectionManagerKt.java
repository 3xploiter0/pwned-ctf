...
public final class SelectionManagerKt {
        ...
        while (index$iv < size) {
            ...
            if (startOffset == endOffset) {
                ...
                if (localCoordinates == null) {
                    ...
                    long containerTopLeft = layoutCoordinates.mo5348localPositionOfR5De75A(localCoordinates, localTopLeft);
                    long containerBottomRight = layoutCoordinates.mo5348localPositionOfR5De75A(localCoordinates, localBottomRight);
                    float containerLeft = Math.min(containerTop, Offset.m3762getXimpl(containerTopLeft));
                    float containerLeft2 = Offset.m3763getYimpl(containerTopLeft);
                    float containerTop2 = Math.min(containerRight, containerLeft2);
                    float containerTop3 = Offset.m3762getXimpl(containerBottomRight);
                    float containerRight3 = Math.max(containerRight2, containerTop3);
                    float containerRight4 = Offset.m3763getYimpl(containerBottomRight);
    ...
    public static final long m1380calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long magnifierSize) {
        ...
        switch (draggingHandle == null ? -1 : WhenMappings.$EnumSwitchMapping$0[draggingHandle.ordinal()]) {
            ...
            case 1:
                return m1382getMagnifierCenterJVtK1S4(manager, magnifierSize, selection.getStart());
            case 2:
                return m1382getMagnifierCenterJVtK1S4(manager, magnifierSize, selection.getEnd());
            case 3:
    ...
    private static final long m1382getMagnifierCenterJVtK1S4(SelectionManager manager, long magnifierSize, Selection.AnchorInfo anchor) {
        LayoutCoordinates containerCoordinates;
        ...
        if (selectable != null && (containerCoordinates = manager.getContainerLayoutCoordinates()) != null && (selectableCoordinates = selectable.getLayoutCoordinates()) != null && (offset = anchor.getOffset()) <= selectable.getLastVisibleOffset()) {
            Offset offsetM1369getCurrentDragPosition_m7T9E = manager.m1369getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1369getCurrentDragPosition_m7T9E);
            long localDragPosition = selectableCoordinates.mo5348localPositionOfR5De75A(containerCoordinates, offsetM1369getCurrentDragPosition_m7T9E.getPackedValue());
            float dragX = Offset.m3762getXimpl(localDragPosition);
            long lineRange = selectable.mo1330getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m5942getCollapsedimpl(lineRange)) {
                lineStartX = selectable.getLineLeft(offset);
            } else {
                float lineStartX2 = selectable.getLineLeft(TextRange.m5948getStartimpl(lineRange));
                float lineEndX = selectable.getLineRight(TextRange.m5943getEndimpl(lineRange) - 1);
            ...
            if (!IntSize.m6653equalsimpl0(magnifierSize, IntSize.INSTANCE.m6660getZeroYbymL2g()) && Math.abs(dragX - lineStartX) > IntSize.m6655getWidthimpl(magnifierSize) / 2) {
                return Offset.INSTANCE.m3777getUnspecifiedF1C5BW0();
            }
            float lineCenterY = selectable.getCenterYForOffset(offset);
            return lineCenterY == -1.0f ? Offset.INSTANCE.m3777getUnspecifiedF1C5BW0() : containerCoordinates.mo5348localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(lineStartX, lineCenterY));
        }
        return Offset.INSTANCE.m3777getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m3800Rect0a9Yr6o($this$visibleBounds.mo5356windowToLocalMKHz9U(boundsInWindow.m3797getTopLeftF1C5BW0()), $this$visibleBounds.mo5356windowToLocalMKHz9U(boundsInWindow.m3791getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m1381containsInclusiveUv8p0NA(Rect $this$containsInclusive_u2dUv8p0NA, long offset) {
        float left = $this$containsInclusive_u2dUv8p0NA.getLeft();
        float right = $this$containsInclusive_u2dUv8p0NA.getRight();
        float fM3762getXimpl = Offset.m3762getXimpl(offset);
        if (left <= fM3762getXimpl && fM3762getXimpl <= right) {
            float top = $this$containsInclusive_u2dUv8p0NA.getTop();
            float bottom = $this$containsInclusive_u2dUv8p0NA.getBottom();
            float fM3763getYimpl = Offset.m3763getYimpl(offset);
            if (top <= fM3763getYimpl && fM3763getYimpl <= bottom) {
                return true;
            }
        }
        return false;
