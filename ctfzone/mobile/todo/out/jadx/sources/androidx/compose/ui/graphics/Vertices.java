...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m4057toArgb8_81llA(colors.get(i).m4013unboximpl());
        }
        return iArr;
    }

    private final float[] encodePointList(List<Offset> points) {
        float fM3763getYimpl;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3763getYimpl = Offset.m3762getXimpl(point);
            } else {
                fM3763getYimpl = Offset.m3763getYimpl(point);
            }
            fArr[i] = fM3763getYimpl;
        }
        return fArr;
    }
}
