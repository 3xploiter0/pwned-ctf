...
public final class PagerSnapLayoutInfoProviderKt {
    public static final SnapLayoutInfoProvider SnapLayoutInfoProvider(final PagerState pagerState, final PagerSnapDistance pagerSnapDistance, final Function3<? super Float, ? super Float, ? super Float, Float> function3) {
        return new SnapLayoutInfoProvider() { // from class: androidx.compose.foundation.gestures.snapping.PagerSnapLayoutInfoProviderKt.SnapLayoutInfoProvider.1
            ...
            public float calculateSnapOffset(float velocity) {
                ...
                Pair<Float, Float> pairSearchForSnappingBounds = searchForSnappingBounds(snapPosition);
            ...
            public float calculateApproachOffset(float velocity, float decayOffset) {
                ...
                int correctedTargetPage = RangesKt.coerceIn(pagerSnapDistance.calculateTargetPage(startPage, targetPage, velocity, pagerState.getPageSize$foundation_release(), pagerState.getPageSpacing$foundation_release()), 0, pagerState.getPageCount());
            ...
            private final Pair<Float, Float> searchForSnappingBounds(SnapPosition snapPosition) {
                ...
                boolean isDragging = !(PagerSnapLayoutInfoProviderKt.dragGestureDelta(pagerState) == 0.0f);
                if (!pagerState.getCanScrollForward()) {
                    upperBoundOffset = 0.0f;
                    if (isDragging && PagerSnapLayoutInfoProviderKt.isScrollingForward(pagerState)) {
    ...
    public static final float dragGestureDelta(PagerState $this$dragGestureDelta) {
        if ($this$dragGestureDelta.getLayoutInfo().getOrientation() == Orientation.Horizontal) {
            return Offset.m3762getXimpl($this$dragGestureDelta.m929getUpDownDifferenceF1C5BW0$foundation_release());
        }
        return Offset.m3763getYimpl($this$dragGestureDelta.m929getUpDownDifferenceF1C5BW0$foundation_release());
    }
    ...
    public static final float calculateFinalSnappingBound(PagerState pagerState, LayoutDirection layoutDirection, float snapPositionalThreshold, float flingVelocity, float lowerBoundOffset, float upperBoundOffset) {
        boolean isForward = (pagerState.getLayoutInfo().getOrientation() == Orientation.Vertical || layoutDirection == LayoutDirection.Ltr) ? isScrollingForward(pagerState) : !isScrollingForward(pagerState);
        ...
        if (FinalSnappingItem.m520equalsimpl0(finalSnappingItem, FinalSnappingItem.INSTANCE.m524getClosestItembbeMdSM())) {
            return Math.abs(offsetFromSnappedPositionOverflow) <= snapPositionalThreshold ? upperBoundOffset : upperBoundOffset;
        }
        if (!FinalSnappingItem.m520equalsimpl0(finalSnappingItem, FinalSnappingItem.INSTANCE.m525getNextItembbeMdSM())) {
            if (!FinalSnappingItem.m520equalsimpl0(finalSnappingItem, FinalSnappingItem.INSTANCE.m526getPreviousItembbeMdSM())) {
                return 0.0f;
            }
