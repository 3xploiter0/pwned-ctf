...
public final class MultiParagraphKt {
    public static final int findParagraphByIndex(List<ParagraphInfo> list, int index) {
        ...
        if (!(index <= ((ParagraphInfo) CollectionsKt.last((List) list)).getEndIndex())) {
            throw new IllegalArgumentException(("Index " + index + " should be less or equal than last line's end " + lastLineEnd).toString());
        }
        int low$iv = 0;
    ...
    public static final void m5820findParagraphsByRangeSbBc2M(List<ParagraphInfo> list, long range, Function1<? super ParagraphInfo, Unit> function1) {
        int paragraphIndex = findParagraphByIndex(list, TextRange.m5946getMinimpl(range));
        int size = list.size();
        for (int i = paragraphIndex; i < size; i++) {
            ParagraphInfo paragraph = list.get(i);
            if (paragraph.getStartIndex() < TextRange.m5945getMaximpl(range)) {
