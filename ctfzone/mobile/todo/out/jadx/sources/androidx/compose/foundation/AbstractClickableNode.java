...
public abstract class AbstractClickableNode extends DelegatingNode implements PointerInputModifierNode, KeyInputModifierNode, FocusEventModifierNode, SemanticsModifierNode, TraversableNode {
    ...
    private AbstractClickableNode(MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, String onClickLabel, Role role, Function0<Unit> function0) {
        ...
        this.centerOffset = Offset.INSTANCE.m3778getZeroF1C5BW0();
    ...
    protected final void m211updateCommonQzZPfjk(MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, String onClickLabel, Role role, Function0<Unit> onClick) {
    ...
    public final void mo209onPointerEventH0pRuoY(PointerEvent pointerEvent, PointerEventPass pass, long bounds) {
        long $this$toOffset_u2d_u2dgyyYBs$iv = IntSizeKt.m6661getCenterozmzZPI(bounds);
        this.centerOffset = OffsetKt.Offset(IntOffset.m6613getXimpl($this$toOffset_u2d_u2dgyyYBs$iv), IntOffset.m6614getYimpl($this$toOffset_u2d_u2dgyyYBs$iv));
        initializeIndicationAndInteractionSourceIfNeeded();
        if (this.enabled && pass == PointerEventPass.Main) {
            int type = pointerEvent.getType();
            if (PointerEventType.m5159equalsimpl0(type, PointerEventType.INSTANCE.m5163getEnter7fucELk())) {
            ...
            } else if (PointerEventType.m5159equalsimpl0(type, PointerEventType.INSTANCE.m5164getExit7fucELk())) {
    ...
    public final boolean mo208onKeyEventZmokQxo(KeyEvent event) {
        initializeIndicationAndInteractionSourceIfNeeded();
        if (this.enabled && Clickable_androidKt.m283isPressZmokQxo(event)) {
            if (this.currentKeyPressInteractions.containsKey(Key.m4757boximpl(KeyEvent_androidKt.m5068getKeyZmokQxo(event)))) {
                return false;
            }
            PressInteraction.Press press = new PressInteraction.Press(this.centerOffset, null);
            this.currentKeyPressInteractions.put(Key.m4757boximpl(KeyEvent_androidKt.m5068getKeyZmokQxo(event)), press);
            if (this.interactionSource == null) {
                return true;
            }
        ...
        if (!this.enabled || !Clickable_androidKt.m281isClickZmokQxo(event)) {
            return false;
        }
        PressInteraction.Press it = this.currentKeyPressInteractions.remove(Key.m4757boximpl(KeyEvent_androidKt.m5068getKeyZmokQxo(event)));
    ...
    public final boolean mo210onPreKeyEventZmokQxo(KeyEvent event) {
        return false;
    }
    ...
    public final void applySemantics(SemanticsPropertyReceiver $this$applySemantics) {
        if (this.role != null) {
            ...
            SemanticsPropertiesKt.m5777setRolekuIjeqM($this$applySemantics, role.getValue());
        }
    ...
    protected final Object m207handlePressInteractiond4ec7I(PressGestureScope $this$handlePressInteraction_u2dd_u2d4ec7I, long offset, Continuation<? super Unit> continuation) {
        Object objCoroutineScope;
        MutableInteractionSource interactionSource = this.interactionSource;
        return (interactionSource == null || (objCoroutineScope = CoroutineScopeKt.coroutineScope(new AbstractClickableNode$handlePressInteraction$2$1($this$handlePressInteraction_u2dd_u2d4ec7I, offset, interactionSource, this, null), continuation)) != IntrinsicsKt.getCOROUTINE_SUSPENDED()) ? Unit.INSTANCE : objCoroutineScope;
    }
