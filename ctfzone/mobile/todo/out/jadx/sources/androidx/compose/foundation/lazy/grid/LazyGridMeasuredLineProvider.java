...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m826childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m6450fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m6449fixedHeightOenEA2s(crossAxisSize);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m804getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).getPackedValue());
            long constraints = m826childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem lazyGridMeasuredItemM825getAndMeasurem8Kt_7k = this.measuredItemProvider.m825getAndMeasurem8Kt_7k(lineConfiguration.getFirstItemIndex() + i, constraints, startSlot, span, mainAxisSpacing);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i] = lazyGridMeasuredItemM825getAndMeasurem8Kt_7k;
        }
