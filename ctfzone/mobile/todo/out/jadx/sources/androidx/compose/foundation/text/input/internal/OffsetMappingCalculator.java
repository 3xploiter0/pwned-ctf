...
public final class OffsetMappingCalculator {
    ...
    private int[] ops = OpArray.m1183constructorimpl(10);
    private int opsSize;
    ...
    public final void recordEditOperation(int sourceStart, int sourceEnd, int newLength) {
        ...
        if (sourceLength >= 2 || sourceLength != newLength) {
            ...
            if (newSize > OpArray.m1190getSizeimpl(this.ops)) {
                int newCapacity = Math.max(newSize * 2, OpArray.m1190getSizeimpl(this.ops) * 2);
                this.ops = OpArray.m1185copyOfpSmdads(this.ops, newCapacity);
            }
            OpArray.m1192setimpl(this.ops, this.opsSize, sourceMin, sourceLength, newLength);
            this.opsSize = newSize;
        }
    }

    /* renamed from: mapFromSource--jx7JFs, reason: not valid java name */
    public final long m1181mapFromSourcejx7JFs(int offset) {
        return m1178mapfzxv0v0(offset, true);
    }

    /* renamed from: mapFromDest--jx7JFs, reason: not valid java name */
    public final long m1180mapFromDestjx7JFs(int offset) {
        return m1178mapfzxv0v0(offset, false);
    }

    /* renamed from: map-fzxv0v0, reason: not valid java name */
    private final long m1178mapfzxv0v0(int offset, boolean fromSource) {
        int start = offset;
        int end = offset;
        ...
        if (max$iv >= 0) {
            if (reversed$iv) {
                ...
                for (int i$iv = max$iv - 1; -1 < i$iv; i$iv--) {
                    ...
                    long newStart = m1179mapStepC6uMEY(start2, offset$iv, srcLen$iv, destLen$iv, fromSource);
                    long newEnd = m1179mapStepC6uMEY(end2, offset$iv, srcLen$iv, destLen$iv, fromSource);
                    start2 = Math.min(TextRange.m5948getStartimpl(newStart), TextRange.m5948getStartimpl(newEnd));
                    end2 = Math.max(TextRange.m5943getEndimpl(newStart), TextRange.m5943getEndimpl(newEnd));
                }
                start = start2;
                end = end2;
            } else {
                ...
                for (int i$iv2 = 0; i$iv2 < max$iv; i$iv2++) {
                    ...
                    long newStart2 = m1179mapStepC6uMEY(start3, offset$iv2, srcLen$iv2, destLen$iv2, fromSource);
                    long newEnd2 = m1179mapStepC6uMEY(end3, offset$iv2, srcLen$iv2, destLen$iv2, fromSource);
                    start3 = Math.min(TextRange.m5948getStartimpl(newStart2), TextRange.m5948getStartimpl(newEnd2));
                    end3 = Math.max(TextRange.m5943getEndimpl(newStart2), TextRange.m5943getEndimpl(newEnd2));
                }
                start = start3;
                end = end3;
            }
    ...
    private final long m1179mapStepC6uMEY(int offset, int opOffset, int untransformedLen, int transformedLen, boolean fromSource) {
