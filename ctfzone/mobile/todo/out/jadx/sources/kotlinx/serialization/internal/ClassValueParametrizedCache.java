...
final class ClassValueParametrizedCache<T> implements ParametrizedSerializerCache<T> {
    ...
    public Object mo8533getgIAlus(KClass<Object> key, List<? extends KType> types) {
        ...
        Object objM6955boximpl = concurrentHashMap.get(arrayList2);
        if (objM6955boximpl == null) {
            try {
                Result.Companion companion = Result.INSTANCE;
                objM6956constructorimpl = Result.m6956constructorimpl(this.compute.invoke(key, types));
            } catch (Throwable th) {
                Result.Companion companion2 = Result.INSTANCE;
                objM6956constructorimpl = Result.m6956constructorimpl(ResultKt.createFailure(th));
            }
            objM6955boximpl = Result.m6955boximpl(objM6956constructorimpl);
            Object objPutIfAbsent = concurrentHashMap.putIfAbsent(arrayList2, objM6955boximpl);
            if (objPutIfAbsent != null) {
                objM6955boximpl = objPutIfAbsent;
            }
        }
        Intrinsics.checkNotNullExpressionValue(objM6955boximpl, "getOrPut(...)");
        return ((Result) objM6955boximpl).getValue();
    }
}
